...
class DenseNet(nn.Module):
    def __init__(self, block, num_block, growth_rate=12, reduction=0.5, num_classes=10,num_channel=3):
        super(DenseNet, self).__init__()
        self.growth_rate = growth_rate

        num_planes = 2 * growth_rate
        self.conv1 = nn.Conv2d(num_channel, num_planes, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        x = self.avg_pool(func.relu(self.bn(x)))
        x = x.view(x.size(0), -1)
        x = self.linear(x)
        return self.sigmoid(x)


def DenseNetStudent(num_classes, num_channel):
    return DenseNet(Bottleneck, [1, 2, 2, 2], growth_rate=4,num_classes=num_classes, num_channel=num_channel)

def DenseNetTeacher(num_classes, num_channel):
    return DenseNet(Bottleneck, [6, 12, 48, 32],growth_rate=32,num_classes=num_classes, num_channel=num_channel)
